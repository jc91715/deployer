            ...
            <el-col :span="12" :offset="6">
                <el-form :model="form" :rules="rules" ref="form" label-position="top">
                    ...
                    <el-form-item label="任务选择" prop="task_ids">
                        <select   class="multi-select" id="recipe_id" multiple="multiple">
                            <option :value="task.id" v-for="(task,index) in tasks" :key="index"  :selected = "form.task_ids.length>0?(form.task_ids.join(',').indexOf(task.id)!==-1?'selected':''):''">{{task.name}}</option>
                        </select>
                    </el-form-item>
    ...
    export default {
        ...
        methods:{
            ...
            async submitForm(formName) {
                ...
                this.loading = true
                const {  space_id,project_id,name,env,server_id,task_ids,repository } = this.form

                await store.dispatch('projectUpdate', { space_id,project_id,name,env,server_id,task_ids,repository})
        ...
        async created(){
            ...
            const data = await store.dispatch('projectShow',{space:this.id,project_id:this.project_id})

            const { space_id,server_id,name, env, repository} = data.data.project
            const form = { space_id,server_id,name, env, repository}
